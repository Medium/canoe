 ...
 *
 * @fires S3Stream#drain
 * @fires S3Stream#flush
 */
S3Stream.prototype.setupEvents = function () {
  ...
  this.on('uploaded', function () {
    /**
     * An upload has finished and the stream is ready for more data.
     *
     * @event S3Stream#drain
     */
    if (_this.ready()) _this.emit('drain');

    /**
     * The stream has no more active uploads.
     *
     * @event S3Stream#flush
     */
 ...
 *
 * @fires S3Stream#error
 * @fires S3Stream#uploaded
 */
S3Stream.prototype.upload = function (body) {
  ...
  this.s3.uploadPart(params, function (err, response) {
    ...
    /**
     * Part has been uploaded
     *
     * @event S3Stream#uploaded
     * @type {Error}
     * @type {Object}
     * @type {Buffer}
     */
